...
def load_config(path=None):
    return json.loads(Path(path or ROOT/'config/rig_sim_v0.2.json').read_text(encoding='utf-8'))
...
def camera_specs(c):
    ...
    for sid,name,cn,pos,f,u,profile,controller in specs:
        ...
        if o['model']=='ideal_pinhole':
            ...
            fy=H/2/math.tan(math.radians(o['vfov_deg']/2))
            ...
            if 'vfov_deg' in o and not math.isclose(vfov,o['vfov_deg'],abs_tol=1e-9):
                raise ValueError('Explicit VFOV incompatible with equidistant focal/radius')
        o.update(fx_px=focal,fy_px=fy,cx_px=cx,cy_px=cy,derived_vfov_deg=vfov,

...
ROOT=Path(__file__).resolve().parents[1];r=ROOT/'reports/step2_1b_v3'
a=np.load(ROOT/'.cache/step2_1b_v3/evaluated_meshes.npz');body={k:a[k] for k in a.files if k.startswith('body')};rig={k:a[k] for k in a.files if k.startswith('rig')}
def topology(tri):

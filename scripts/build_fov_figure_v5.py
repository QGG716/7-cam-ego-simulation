"""V5 layout only: existing native RGB and external renders, no simulation or image processing."""
...
OUT=ROOT/'outputs/step2_1b_v5';REPORT=ROOT/'reports/step2_1b_v5';CONF=ROOT/'config/step2_1b_v5'
SOURCE=ROOT/'outputs/step2_1b_v4/raw';EXTERNAL=ROOT/'outputs/step2_1b_v3/debug'
...
def main():
 ...
 rigpath=ROOT/'config/rig_sim_v0.2.json';fitpath=ROOT/'config/step2_1b_v3/final_placement.json';rig=read(rigpath);p=read(fitpath);views=read(EXTERNAL/'external_views.json');records={r['camera_id']:r for r in p['camera_records']};R=np.array(p['R_world_B']);t=np.array(p['rig_translation_world_m'])
 ...
 for group,ids in GROUPS.items():
  for cid in ids:
   ...
   shutil.copyfile(source,destination);assert source.read_bytes()==destination.read_bytes()
   old=EXTERNAL/(cid+'_scene.png');assert sha(old)==sha(source)
   manifest[cid]=dict(group=group,source=source.relative_to(ROOT).as_posix(),output=destination.relative_to(ROOT).as_posix(),sha256=sha(source),v3_source_sha256=sha(old),resolution=size,mode=mode,copy_byte_identical=True)
 ...
 for group,title in [('B','前向三目  C1 / C0 / C2'),('C','上视两目  C3 / C5'),('D','下视两目  C4 / C6')]:
  ...
  if group=='B':
   ...
   s=optics['surround'];text(x+40,y+1240,f"{s['resolution'][0]}×{s['resolution'][1]} · 等距鱼眼 · 圆形成像 {s['radial_full_fov_deg']:g}° · 直径 {s['image_circle_diameter_px']:g} px · 最大离轴 {s['radial_full_fov_deg']/2:g}°",13)
 ...
 for ext in ['png','pdf','svg']:fig.savefig(OUT/('fov_overview_wearer_v5.'+ext),dpi=200,facecolor='white')
 svg=OUT/'fov_overview_wearer_v5.svg';svg.write_text('\n'.join(s.rstrip() for s in svg.read_text(encoding='utf-8').splitlines())+'\n',encoding='utf-8')
 ...
 config=dict(baseline_commit='e7f88b83c0ebf98f6af2bf08fa18748163b8e54e',groups=GROUPS,canvas_px=[W,H],panels=panels,image_processing='none; proportional layout scaling only',camera_image_overlays=False,source_configuration={s.relative_to(ROOT).as_posix():sha(s) for s in sources},optics=rig['optics'],camera_height_above_floor_m={k:r['height_above_floor_m'] for k,r in records.items()})
 (CONF/'figure_config.json').write_text(json.dumps(config,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 (REPORT/'image_manifest.json').write_text(json.dumps(dict(images=manifest,configuration=config['source_configuration'],meaning='Existing native per-camera simulation RGB, not Bayer RAW; pre-existing valid-domain treatment retained; no processing or image transforms',basic_checks=dict(seven_images=True,byte_identical_copies=True,equal_aspect_full_frames=True,no_image_overlay_artists=True,no_label_image_intersection=True),runtime='Layout only; no old builder import, overlap entry point, Isaac, new acquisition or direction experiments'),ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print('V5 exported: PNG PDF SVG, seven byte-identical native RGB copies, four enlarged panels; basic layout checks PASS')

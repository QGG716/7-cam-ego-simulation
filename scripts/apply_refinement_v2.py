...
try:
 ...
 stage=Usd.Stage.Open(str(ROOT/'scenes/step2_1b_v2/indoor_worker.usda'))
 ...
 cfg=json.loads((ROOT/'config/step2_1b_v2/refined_mount.json').read_text());mesh=UsdGeom.Mesh(stage.GetPrimAtPath('/World/Rig/WearerMount/FittedBand'));vertices=np.array(cfg['vertices_B_m']);indices=np.array(cfg['triangles']);mesh.GetPointsAttr().Set(Vt.Vec3fArray(vertices.tolist()));mesh.GetFaceVertexCountsAttr().Set([3]*len(indices));mesh.GetFaceVertexIndicesAttr().Set(indices.reshape(-1).tolist());mesh.CreateExtentAttr().Set([Gf.Vec3f(*vertices.min(0)),Gf.Vec3f(*vertices.max(0))]);stage.GetPrimAtPath('/World/Rig/WearerMount/ForeheadBridge').SetActive(False);stage.GetRootLayer().Save()
 path=ROOT/'config/step2_1b_v2/final_placement.json';p=json.loads(path.read_text());p['worker']['appearance']=dict(method='Official construction worker geometry retained; user-authorized industrial palette on apparel/PPE; original skin material retained; source asset read-only',palette=palette)
 ...
 cachepath=ROOT/'.cache/step2_1b_v2/evaluated_meshes.npz';a=np.load(cachepath);data={k:a[k] for k in a.files if k!='rig14'};m=np.array(UsdGeom.Xformable(mesh).ComputeLocalToWorldTransform(0));actual=np.array(mesh.GetPointsAttr().Get())@m[:3,:3]+m[3,:3];data['rig13']=actual[indices];np.savez_compressed(cachepath,**data)
 body={k:v for k,v in data.items() if k.startswith('body')};rows=json.loads((ROOT/'reports/step2_1b_v2/fit_after.json').read_text());rows=[r for r in rows if '/WearerMount/' not in r['path']]+audit({'/World/Rig/WearerMount/FittedBand':data['rig13']},body);(ROOT/'reports/step2_1b_v2/fit_after.json').write_text(json.dumps(rows,indent=2));p['fit_status']='PASS' if not any(r.get('triangle_crossings',0) or r.get('human_vertices_inside_solid',0) for r in rows) else 'FAIL';path.write_text(json.dumps(p,indent=2));print('REFINEMENT_APPLIED',p['fit_status'],flush=True)
except Exception as e:
 import traceback
 (ROOT/'reports/step2_1b_v2/refinement_error.json').write_text(json.dumps(dict(error=str(e),traceback=traceback.format_exc()),indent=2));raise
finally:app.close()

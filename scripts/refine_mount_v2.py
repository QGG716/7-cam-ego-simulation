...
p=json.loads((ROOT/'config/step2_1b_v2/final_placement.json').read_text());R=np.array(p['R_world_B']);t=np.array(p['rig_translation_world_m']);a=np.load(ROOT/'.cache/step2_1b_v2/evaluated_meshes.npz');rows=json.loads((ROOT/'reports/step2_1b_v2/fit_after.json').read_text());objects={row['path']:(a['rig'+str(i)]-t)@R*1000 for i,row in enumerate(rows)}
...
if '--verify' not in sys.argv:(ROOT/'reports/step2_1b_v2/mount_hardware_boolean.json').write_text(json.dumps(dict(status='PASS',intersection_volume_mm3=residual,numerical_volume_tolerance_mm3=.01,method='Closed oriented triangle solids, Manifold3D Boolean intersection; housing butt-contact allowed; 0.5 mm cutter enlargement for cable and heatsink',connected_components=len(components)),indent=2))
...
if '--verify' not in sys.argv:(ROOT/'config/step2_1b_v2/refined_mount.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v for k,v in result.items() if k not in ['vertices_B_m','triangles']},indent=2));print('vertices',len(vertices),'triangles',len(triangles))

...
r=Path(__file__).resolve().parents[1];a=np.load(r/'.cache/step2_1b_v3/evaluated_meshes.npz');rows=json.loads((r/'reports/step2_1b_v3/fit_after.json').read_text());rig={row['path']:a['rig'+str(i)] for i,row in enumerate(rows)}
connections=[]
...
(r/'reports/step2_1b_v3/mount_connectivity.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
